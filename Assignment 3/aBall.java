...
public class aBall extends Thread {//thread can make 100 balls move at the same time
     GOval myBall; //create the object myBall and make it accessible by bSim class
     double Xi;           
	 double Yi;           
	double Vo;          
	double theta;       
...
public void run() {                       //run the simulation of the ball
	...
	//double Elast=0.5*Vo*Vo;
	//KEx+KEy<ETHR || KEx+KEy>=Elast
	...
	try {
		Thread.sleep(1);    //a2 the sleep duration is 1ms
